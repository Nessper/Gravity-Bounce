...
public class BootRoot : MonoBehaviour
{
    ...
    /// <summary>
    /// Appele par AudioManager pour s enregistrer.
    /// </summary>
    public static void RegisterAudio(AudioManager manager)
    {
        if (Audio != null && Audio != manager)
        {
            Debug.LogWarning("[BootRoot] Multiple AudioManager detected. Keeping the first one.");
            return;
        }

...
public class BoardOutroAssembler : MonoBehaviour
{
    ...
    // BoardOutroAssembler.cs
    // Ajout en bas (API publique)

    /// <summary>
    /// Force instantanément l'état final de l'outro (comme si PlayOutro() avait fini).
    /// Utile pour la reprise de cérémonie après crash (snapshot).
    /// </summary>
    public void ForceOutroStateInstant()
    {
        // S'assurer que les caches sont prêts (au cas où)
        InitializeVisualDataIfNeeded();
        InitializePlayerDataIfNeeded();
        InitializeBinDataIfNeeded();
        ...
        // 2) Walls + board invisibles + désactivés (comme fin de PlayWallsAndBoardShrink)
        if (wallsRoot != null)
            wallsRoot.gameObject.SetActive(false);
        ...
        // 3) Bins invisibles + désactivés (comme fin de PlayBinsFadeOut)
        SetBinsAlpha(0f);
